...
const LAYOUT: DistrictLayout[] = [
  { key: 'bpmh', door: { x: 300, y: 340 }, x0: 430, y0: 392, w: 170, h: 0, rows: 1,
    plaque: { x: 470, y: 288, side: 'right' }, seed: 1000 },
  { key: 'oncology', door: { x: 1660, y: 340 }, x0: 1618, y0: 404, w: -410, h: 150, rows: 3,
    plaque: { x: 1136, y: 626, side: 'right' }, seed: 1077 },
  { key: 'cpoe', door: { x: 300, y: 802 }, x0: 404, y0: 686, w: 376, h: 200, rows: 3,
    plaque: { x: 832, y: 972, side: 'left' }, seed: 1154 },
  { key: 'verification', door: { x: 1650, y: 822 }, x0: 1596, y0: 642, w: -436, h: 264, rows: 4,
    plaque: { x: 1120, y: 976, side: 'right' }, seed: 1231 },
];
...
const HOUSES: Record<JobAidId, HouseCfg> = {
  bpmh: { x: 250, y: 330, flip: false, smokeDelay: '7s', nameLines: ['BPMH &', 'MED REC'], nameY: [-92, -77] },
  oncology: { x: 1690, y: 330, flip: true, smokeDelay: '8.5s', nameLines: ['ONCOLOGY', 'ORDERS'], nameY: [-92, -77] },
  cpoe: { x: 250, y: 792, flip: false, smokeDelay: '7.8s', nameLines: ['CPOE'], nameY: [-84] },
  verification: { x: 1690, y: 812, flip: true, smokeDelay: '9s', nameLines: ['PHARMACIST', 'VERIFICATION'], nameY: [-92, -77] },
};
...
export function Overworld({ workflows, completed, onSelect }: OverworldProps) {
  ...
  const stars = (() => {
    ...
    const out: { x: number; y: number; r: number; twinkle: boolean; dur: number; delay: number; opacity: number }[] = [];
  ...
  LAYOUT.forEach((L) => {
    ...
    const done = Math.min(list.filter((w) => completed.includes(w.id)).length, n);
    totalDone += done;
    totalAll += n;
    ...
    trails.push({ key: L.key, color, d: crPath(pts), lit: crPath(pts.slice(0, done + 1)) });

    for (let i = 0; i < n; i++) {
      const p = pts[i + 1];
      const state: NodeState = i < done ? 'cleared' : i === done ? 'current' : 'sealed';
      nodes.push({
        key: `${L.key}${i}`,
        ...
        y: +p.y.toFixed(1),
        state,
        stone: state === 'sealed' ? '#241a42' : state === 'current' ? color : color + '66',
        edge: state === 'sealed' ? '#4a3d63' : state === 'current' ? '#caa14a' : color,
        edgeW: state === 'current' ? 2.6 : 1.6,
        rune: state === 'sealed' ? '#4a3d63' : '#f4ead6',
        num: state === 'sealed' ? '#4a3d63' : state === 'current' ? '#f4ead6' : '#c9b8e8',
    ...
    if (done < n) {
      ...
      plaques.push({
        ...
        frame: right ? `M0,-32 L${w},-32 L${w},42 L0,42 Z` : `M${-w},-32 L0,-32 L0,42 L${-w},42 Z`,
  ...
  const hidden = new Set(
    nodes.filter((n) => plaqueRects.some((r) => n.x > r.x1 && n.x < r.x2 && n.y > r.y1 && n.y < r.y2)).map((n) => n.key),
  );
      ...
      >
        ...
        >
          <defs>
            ...
            <filter id="ow-softblur" x="-60%" y="-60%" width="220%" height="220%">
              <feGaussianBlur stdDeviation="9" />
            </filter>
          </defs>

          <rect x="0" y="0" width={STAGE_W} height={STAGE_H} fill="url(#ow-sky)" />
          ...
          {stars.map((s, i) => (
            <circle
              ...
              style={s.twinkle ? ({ '--dur': `${s.dur}s`, '--delay': `${s.delay}s`, opacity: 0.9 } as React.CSSProperties) : undefined}
            />
          ))}
          ...
          <path
            ...
            opacity="0.92"
          />
          <path d="M0,272 L0,238 L1920,238 L1920,272 Z" fill="#160f28" opacity="0.6" />

          <rect x="0" y="248" width={STAGE_W} height="832" fill="url(#ow-ground)" />
          <ellipse cx="960" cy="262" rx="1180" ry="52" fill="url(#ow-mist)" />
          <ellipse cx="520" cy="286" rx="620" ry="34" fill="url(#ow-mist)" opacity="0.7" />
          <g opacity="0.5">
          ...
          <g>
            <circle cx="960" cy="352" r="176" fill="url(#ow-lamp)" opacity="0.5" />
            <ellipse cx="960" cy="574" rx="118" ry="26" fill="#120c22" opacity="0.7" />
            <path d="M898,566 L906,352 L1014,352 L1022,566 Z" fill="#2a1d47" stroke="#4a3d63" strokeWidth="2" />
            <path d="M906,352 L1014,352 L1014,340 L906,340 Z" fill="#372753" />
            <path d="M890,340 L1030,340 L1022,318 L898,318 Z" fill="#372753" stroke="#4a3d63" strokeWidth="1.5" />
            <path d="M898,318 L960,232 L1022,318 Z" fill="#241a42" stroke="#4a3d63" strokeWidth="2" />
            <path d="M960,232 L960,206" stroke="#caa14a" strokeWidth="3" />
            <circle cx="960" cy="200" r="6" fill="#caa14a" />
            <path d="M938,382 L982,382 L982,428 Q960,444 938,428 Z" fill="#ffcf8f" opacity="0.9" filter="url(#ow-glow)" />
            <path d="M926,470 L994,470 L994,516 L926,516 Z" fill="#caa14a" opacity="0.22" />
            <path d="M940,528 L980,528 L980,566 L940,566 Z" fill="#160f28" stroke="#4a3d63" strokeWidth="1.5" />
            <text x="960" y="622" textAnchor="middle" fontFamily="Cinzel, serif" fontSize="21" letterSpacing="4" fill="#caa14a">ALDRIC&apos;S TOWER</text>
            <text x="960" y="648" textAnchor="middle" fontFamily="Spectral, serif" fontSize="16" letterSpacing="1.5" fill="#7d6bab" fontStyle="italic">the master is in</text>
          </g>

          {trails.map((t) => (
            <g key={t.key}>
              <path d={t.d} fill="none" stroke="#120c22" strokeWidth="16" strokeLinecap="round" opacity="0.55" />
              <path d={t.d} fill="none" stroke="#372753" strokeWidth="9" strokeLinecap="round" />
              <path d={t.d} fill="none" stroke="#4a3d63" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="3 13" opacity="0.8" />
              <path d={t.lit} fill="none" stroke={t.color} strokeWidth="13" strokeLinecap="round" opacity="0.28" filter="url(#ow-softblur)" />
              <path d={t.lit} fill="none" stroke={t.color} strokeWidth="8.5" strokeLinecap="round" />
              <path d={t.lit} fill="none" stroke="#f4ead6" strokeWidth="2.2" strokeLinecap="round" strokeDasharray="2 11" opacity="0.55" />
            </g>
          ))}
          ...
          {nodes.map((n) => (
            <g
              ...
              aria-label={n.state !== 'sealed' ? `${n.title} — ${n.state}` : undefined}
              aria-disabled={n.state === 'sealed'}
              onClick={n.state !== 'sealed' ? () => onSelect(n.workflowId) : undefined}
              onKeyDown={
                n.state !== 'sealed'
                  ? (e) => {
            ...
            >
              <circle className="ow-node-halo" cx="0" cy="-6" r="46" fill="url(#ow-lamp)" opacity={n.halo} />
              <ellipse cx="0" cy="11" rx="17" ry="5.5" fill="#120c22" opacity="0.6" />
              <path d="M-11,10 L-10,-9 Q0,-20 10,-9 L11,10 Z" fill={n.stone} stroke={n.edge} strokeWidth={n.edgeW} />
              <path d="M-6,-4 L6,-4" stroke={n.rune} strokeWidth="1.4" opacity="0.5" />
              <g opacity={n.lantern}>
                <path d="M0,-20 L0,-30 L13,-30" stroke="#caa14a" strokeWidth="1.6" fill="none" />
                <path d="M13,-30 L13,-25" stroke="#caa14a" strokeWidth="1.4" />
                <path d="M9,-25 L17,-25 L18,-15 L8,-15 Z" fill="#3a2a1a" stroke="#caa14a" strokeWidth="1.2" />
                <circle cx="13" cy="-20" r="3.4" fill="#ffd79a" filter="url(#ow-glow)" />
              </g>
              <g opacity={n.currentOn}>
                <circle className="ow-pulse-ring" cx="0" cy="-2" r="26" fill="none" stroke="#caa14a" strokeWidth="1.6" />
                <path d="M0,-22 L0,-44" stroke="#caa14a" strokeWidth="2" />
                <path d="M-7,-44 L7,-44 L9,-30 L-9,-30 Z" fill="#4a3520" stroke="#caa14a" strokeWidth="1.6" />
                <circle cx="0" cy="-37" r="5" fill="#ffd79a" filter="url(#ow-glow)" />
              </g>
            </g>
          ))}
          ...
          {plaques.map((p) => (
            <g key={p.key} transform={`translate(${p.x} ${p.y})`}>
              <path d={p.leader} fill="none" stroke="#caa14a" strokeWidth="1.2" strokeDasharray="4 6" opacity="0.55" />
              <path d={p.frame} fill="#1b1230" stroke="#caa14a" strokeWidth="1.4" opacity="0.96" />
            </g>
          ))}

          {(['bpmh', 'oncology', 'cpoe', 'verification'] as JobAidId[]).map((key) => (
            <DistrictHouse key={key} district={key} cfg={HOUSES[key]} color={theme.jobAids[key].color} />
          ))}

          {/* HUD — wordmark, top-left */}
          <g transform="translate(56 60)">
            <path d="M0,0 L0,-14 L26,-14" stroke="#caa14a" strokeWidth="2" fill="none" />
            <path d="M0,0 L0,50 L26,50" stroke="#caa14a" strokeWidth="2" fill="none" />
            <text x="18" y="16" fontFamily="Cinzel, serif" fontSize="19" fontWeight="700" letterSpacing="4.5" fill="#caa14a">MASTER ALDRIC&apos;S</text>
            <text x="18" y="40" fontFamily="Cinzel, serif" fontSize="19" fontWeight="700" letterSpacing="4.5" fill="#f4ead6">LABYRINTH</text>
          </g>

          {/* HUD — total, top-right */}
          <g transform="translate(1864 60)">
            <path d="M0,0 L0,-14 L-26,-14" stroke="#caa14a" strokeWidth="2" fill="none" />
            <path d="M0,0 L0,50 L-26,50" stroke="#caa14a" strokeWidth="2" fill="none" />
            <text x="-18" y="12" textAnchor="end" fontFamily="Cinzel, serif" fontSize="13" letterSpacing="3" fill="#7d6bab">WORKFLOWS LEARNED</text>
          </g>
        </svg>
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          ...
          {plaques.map((p) => (
            <div key={p.key} style={{ position: 'absolute', left: p.boxLeft, top: p.boxTop, width: p.boxWidth }}>
              <div style={{ font: '500 11px Cinzel, serif', letterSpacing: '2.4px', color: p.tint }}>{p.kicker}</div>
              <div style={{ font: '400 17px/1.35 Spectral, serif', color: '#f4ead6', marginTop: 3 }}>{p.label}</div>
            </div>
          ))}

          {(['bpmh', 'cpoe', 'oncology', 'verification'] as JobAidId[]).map((key) => {
          ...
          <div style={{ position: 'absolute', right: 74, top: 78, font: '700 24px Cinzel, serif', letterSpacing: '2px', color: '#caa14a' }}>
            {totalDone} / {totalAll}
          </div>
        </div>
...
function DistrictHouse({ district, cfg, color }: { district: JobAidId; cfg: HouseCfg; color: string }) {
  ...
  return (
    <g transform={`translate(${cfg.x} ${cfg.y})`}>
      ...
      <path d="M-56,0 L-56,-84 L56,-84 L56,0 Z" fill="#2a1d47" stroke={color} strokeWidth="2" />
      <path d="M-72,-84 L0,-136 L72,-84 Z" fill="#241a42" stroke={color} strokeWidth="2.5" />
      <path d="M-72,-84 L72,-84 L72,-76 L-72,-76 Z" fill={color} opacity="0.45" />
      <rect x="-40" y="-64" width="26" height="24" fill="#ffcf8f" opacity="0.85" filter="url(#ow-glow)" />
      <rect x="14" y="-64" width="26" height="24" fill="#ffcf8f" opacity="0.85" filter="url(#ow-glow)" />
      <path d="M-13,0 L-13,-34 Q0,-44 13,-34 L13,0 Z" fill="#3a2a1a" stroke="#caa14a" strokeWidth="1.6" />
      ...
      <path
        ...
        strokeWidth="1.6"
      />
      <g fill="#c9b8e8" opacity="0.16" className="ow-smoke" style={{ animationDuration: cfg.smokeDelay }}>
      ...
      <path d={`M${lanternX},-6 L${lanternX},-56`} stroke="#4a3d63" strokeWidth="3" />
      <path
        ...
        strokeWidth="1.4"
      />
      <circle cx={lanternX} cy="-49" r="4.5" fill="#ffd79a" filter="url(#ow-glow)" />
...
function DistrictSigil({ district, color }: { district: JobAidId; color: string }) {
  if (district === 'bpmh') {
    // mortar & pestle
    return (
  ...
  return (
    <g stroke={color} strokeWidth="1.8" fill="none">
      <path d="M-8,-11 L8,-11 L1,-1 L1,11 L-1,11 L-1,-1 Z" fill={color} opacity="0.35" />
      <path d="M-8,-11 L8,-11" />
      <path d="M-9,12 L9,12" />
    </g>
  );
